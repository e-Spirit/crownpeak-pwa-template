...
=== CI
We use husky to enforce branch protection rules to prevent direct commits and pushes to the alpha and main branches. On your local machine this variable is not needed, but in your CI it can be set to 1 to allow the semantic release plugin to do releases. This is done automatically by Github.
