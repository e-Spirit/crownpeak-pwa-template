...
=== CI
We use husky to enforce branch protection rules to prevent direct commits and pushes to the branches alpha and master. On your local machine this variable is not needed, but in your CI it can be set to 'yes' to allow the semantic release plugin to do releases.
